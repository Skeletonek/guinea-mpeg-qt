...
- **qmlformat is opt-in**: run-lint.sh never touches qmlformat by default (and CI only runs qmllint). Use `--format` for the equality check or `--format-fix` to auto-apply `qmlformat -i -f`. Reason: qmlformat output is version-sensitive — the host's Qt 6.11.1 keeps blank lines / adds semicolons that Debian trixie's 6.8.2 collapses, so one tree can't satisfy both. If you do run `--format`/`--format-fix`, do it inside the `debian-trixie` distrobox (qmlformat 6.8.2) to match the CI-aligned baseline. CI lint uses `fedora:44` (qmllint 6.11.1); `.qmllint.ini` at repo root sets `UnqualifiedAccess=disable` (the codebase's accepted unqualified-access convention, so CI and IDE lint stay quiet — on Arch the bare `qmllint` is Qt 5's from `qt5-declarative`, while the real Qt 6 tool is `/usr/lib/qt6/bin/qmllint`). run-lint.sh resolves qmllint/qmlformat preferring system Qt 6 bin dirs, then the Fedora `*-qt6` names, then plain `command -v`. qmllint cannot resolve the C++ `GuineaMpeg` module, so a handful of `[import]`/`[missing-property]`/`[unresolved-alias]` warnings always remain and currently fail the QML lint.
