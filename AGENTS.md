...
- `FileDialog.selectedFile` is `file://` — strip prefix only for C++ paths, build URLs with `encodeURI()`. `FileDialog.currentFolder` is unset on first open — never build `currentFile` from it; only set it to an absolute path when the folder is known.
...
- Rust: `cargo fmt --all -- --check` + `cargo clippy --all-targets -- -D warnings`. QML: `qmllint -I qml` over everything — fails on ANY output (errors, warnings or info messages), since a clean Qt 6 qmllint prints nothing — + per-file `qmlformat` equality on `.qml` only (Qt 6.11 qmlformat exits 1 on standalone `.js` even when valid). C++: `clang-format --dry-run --Werror` over `src/` + `tests/cpp/`; clang-tidy only `src/*.cpp` (headers standalone flag `MpvItem::position`, an intentional QML property name).
- **qmlformat is opt-in**: run-lint.sh never touches qmlformat by default (and CI only runs qmllint). Use `--format` for the equality check or `--format-fix` to auto-apply `qmlformat -i -f`. Reason: qmlformat output is version-sensitive — the host's Qt 6.11.1 keeps blank lines / adds semicolons that Debian trixie's 6.8.2 collapses, so one tree can't satisfy both. If you do run `--format`/`--format-fix`, do it inside the `debian-trixie` distrobox (qmlformat 6.8.2) to match the CI-aligned baseline. CI lint uses `fedora:44` (qmllint 6.11.1); `.qmllint.ini` at repo root sets `UnqualifiedAccess=disable` (accepted convention) plus `ImportFailure`/`UnresolvedAlias`/`UnresolvedType`=disable — qmllint cannot resolve the C++ `GuineaMpeg` module (`MpvItem`/`GuineaMpegBackendExt` are unknown types to it). Cross-window refs (`appWindow`/`mainWindow`) are `property var` so qmllint doesn't flag their custom members as `[missing-property]`. On Arch the bare `qmllint` is Qt 5's from `qt5-declarative`, while the real Qt 6 tool is `/usr/lib/qt6/bin/qmllint`; run-lint.sh resolves qmllint/qmlformat preferring system Qt 6 bin dirs, then the Fedora `*-qt6` names, then plain `command -v`. QML lint is fully clean (0 messages).
