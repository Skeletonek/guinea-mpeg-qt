...
- **qmlformat is opt-in**: run-lint.sh never touches qmlformat by default (and CI only runs qmllint). Use `--format` for the equality check or `--format-fix` to auto-apply `qmlformat -i -f`. Reason: qmlformat output is version-sensitive — the host's Qt 6.11.1 keeps blank lines / adds semicolons that Debian trixie's 6.8.2 collapses, so one tree can't satisfy both. If you do run `--format`/`--format-fix`, do it inside the `debian-trixie` distrobox (qmlformat 6.8.2) to match the CI-aligned baseline. CI lint uses `fedora:44` (qmllint 6.11.1, matching the host), where "Unqualified access" is opt-in (`-U`); run-lint.sh resolves qmllint/qmlformat as either `qmllint` or the Fedora `qmllint-qt6` name.
