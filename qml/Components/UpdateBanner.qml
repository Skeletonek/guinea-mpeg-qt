...
InfoBanner {
    ...
    text: buildInfo.debugBuild ? "Development build of GuineaMPEG" : qsTr("A new version of GuineaMPEG is available")
    subText: buildInfo.debugBuild ? buildInfo.buildDate : "%1 → %2".arg(buildInfo.version).arg(root.latestVersion)
    Component.onCompleted: {
        if (buildInfo.debugBuild === true)
            return ;
