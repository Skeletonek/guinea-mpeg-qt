...
Dialog {
    ...
    onOpened: {
        ...
        for (var i = 0; i < root.profileNames.length; i++)
            profilesModel.append({ name: root.profileNames[i], checked: root.profileNames[i] === root.defaultCheckedName })
        root.refreshCount()
    }
    ...
    function refreshCount() {
        ...
        for (var i = 0; i < profilesModel.count; i++)
            if (profilesModel.get(i).checked) n++
        selectedCount = n
    }
    ...
    function selectedNames() {
        var arr = []
        for (var i = 0; i < profilesModel.count; i++)
            if (profilesModel.get(i).checked) arr.push(profilesModel.get(i).name)
        return arr
    }
    ...
    ColumnLayout {
        ...
        Label {
            text: root.profileNames.length === 0
                ? qsTr("No user-created profiles to export.")
                : qsTr("Select profiles to export:")
            color: theme.text
...
Button {
            ...
            onClicked: {
                ...
                fileDialog.currentFile = (folder && String(folder).indexOf("undefined") < 0)
                    ? folder + "/" + name
                    : name
                fileDialog.open()
            }
